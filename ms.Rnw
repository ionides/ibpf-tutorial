...
Figure~\ref{fig:m_slice_g_plot} suggests that the BPF likelihood approximation leads to a lower-bias estimate of $g_u$ than for the coupling parameter $\rho$ in the Gaussian model (Figure~\ref{fig:ibpf-prof-rho-plot}).
