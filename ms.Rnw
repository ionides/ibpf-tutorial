...
The profile took \Sexpr{myround(as.double(prof1time,units="mins"),2)} mins using \Sexpr{prof1cores} computing cores.
...
For this model, we see that there is noticeable discrepancy between the true profile and that obtained using BPF for likelihood evaluation.
This bias is stronger for $\rho$  than $\sigma$, and may lead to parameter estimation bias for IBPF.
...
The $g_\Sexpr{m_g_prof_u}$ slice took \Sexpr{myround(as.double(m_slice_g_time,units="mins"),2)} mins using \Sexpr{m_slice_cores} computing cores.
