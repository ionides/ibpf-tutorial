...
stew(file=paste0(out_dir,"m_prof_R02.rda"),seed=722,{
...
m_R0 <- m
u <- 2
R0_start <- seq(from=25,to=35,length=U)
...
The $R0_2$ profile took \Sexpr{myround(as.double(m_prof_R02time,units="mins"),2)} mins using \Sexpr{cores} computing cores.
Likelihood evaluation took \Sexpr{myround(as.double(m_prof_R02eval_time,units="mins"),2)}, shared between BPF and PF.

<<m-prof-R02-plot, fig.height=8, fig.width=6, out.width="5.5in", fig.cap = "Top: profile for $R0_2$ using an IBPF search with likelihood computed using BPF, for $K=4$ blocks each having 1 unit. Bottom: The same IBPF search with likelihood computed using the particle filter.",eval=T,echo=F>>= 

par(mfrow=c(2,1))
par(mai=c(0.9,0.9,0.1,0.2))

##### window if needed to avoid extremes with filter failures
