...
\date{\small This version compiled on \today,  using \proglang{R} \Sexpr{paste0(R.version$major,".",R.version$minor)}, \pkg{spatPomp} \Sexpr{packageVersion('spatPomp')}, and \pkg{pomp} \Sexpr{packageVersion('pomp')}\\
\vspace{3mm}
Source code at \url{https://github.com/ionides/ibpf-tutorial}\\
\vspace{3mm}
R script at \url{https://github.com/ionides/ibpf-tutorial/blob/main/ms.R}}
...
This study uses a small number of units ($U=\Sexpr{U}$) so that the particle filter is numerically tractable.
...
<<bm_logLik_code,cache=F,echo=T,eval=F,purl=F>>=
...
<<ibpf-mle-code,eval=F,echo=T,purl=F>>=
...
However, for a relatively small number of units ($U=4$) the particle filter provides an adequate approximation.
...
<<model-he10-code,eval=F,echo=T,purl=F>>=
...
Recall that the bias-variance trade-off for likelihood evaluation becomes a tradeoff between two sources of bias for log-likelihood evaluation, due to Jensen's inquality.
...
This is in contrast to the results in Table~\ref{tab:lik-eval} for the spatially correlated random walk example in Section~\ref{sec:bm}.
...
A profile likelihood for one parameter is presented to support this, in Figure~\ref{fig:m-prof-R0-plot}.
...
It may be interesting to know if less Monte Carlo effort would be sufficient, or if greater effort would be beneficial.
...
A useful diagnostic quantity for the particle filter is the effective sample size (ESS) \citep{doucet11,liu01}.
In the notation of \citet{king16}, the filter observation weight for particle $j$ at time $n$ is $w(n,j)=f_{Y_n|X_n}(y_n^*|X^P_{n,j};\theta)$, the normal weight is $\tilde{w}(n,j)=w(n,j)\big/ \sum_{k=1}^Jw(n,k)$, and the effective sample size is
\begin{equation}
...
This bypasses consideration of whether or not ESS is a good estimator. 
Also, the Monte Carlo replicates, denoted by $\hat\ell_n^{(r)}$, for $r=1,\dots,R$, give rise to an improved estimate of the conditional log-likelihood,
\begin{equation}
\label{eq:ell-bar}
\bar \ell_n = \frac{1}{R} \sum_{r=1}^R \hat\ell_n^{(r)},
\end{equation}
together with an uncertainty estimate via the central limit theorem.
One could also replace the arithmetic average  in (\ref{eq:ell-bar}) using \code{pomp::logmeanexp}.
...
We see that both measures agree on the most problematic observations.
However, ESS is not a sensitive proxy for CLLV elsewhere in the distribution.
...
Using the notation of \citet{asfaw23arxiv}, the weights for block ${\mathcal B}_k\subset \seq{1}{U}$ are
\begin{equation}
...
\end{equation}
For our measles spatPomp example, we calculate the BCLL and its variance (BCLLV) as follows:
<<bpfilter-diag-code,eval=F,echo=T,purl=F>>=
...
Log-likelihood anomalies can be plotted against any covariate of interest for exploratory data analysis.
This is analogous to residual analysis for linear models.
...
The likelihood rises toward the maximum and then stabilizes in a neighborhood of the maximum while the estimated parameters explore this region.

<<traces,fig.height=6.5, fig.width=6.5, out.width="6.5in", fig.cap = "Trace plots for the correlated Gaussian random walk example. Constant traces for $\\sigma_1$, $\\tau_1$ and $X_{0,1}$ arise because these parameters are treated as known and not estimated. Note that the values of $\\sigma_1$, $\\tau_1$ and $X_{0,1}$  are shared across units (e.g., there is no $\\sigma_2$) since the \\code{bm2} constructor is called with $\\rho$ as the only unit-specific parameter.">>=
...
Further examples of \code{ibpf} being used for data analysis are provided by \citet{li24} and \citet{wheeler24}.
\citet{li24} studied the spread of COVID-19 in China at the start of the pandemic.
Their models are available in the \code{metapoppkg} R package (\url{https://doi.org/10.5281/zenodo.10149233}) and the article source code is available at \url{https://doi.org/10.5281/zenodo.10149258}.
\citet{wheeler24} studied a cholera epidemic in Haiti.
Their models are available in the \code{haitipkg} R package (\url{https://doi.org/10.5281/zenodo.7557099}) and the article source code is available at \url{https://doi.org/10.5281/zenodo.10783080}.
