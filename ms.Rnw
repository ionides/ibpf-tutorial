...
\section{Correlated Gaussian random walks} \label{sec:bm}
...
ibpf_prof_searches <- foreach(s=1:nrow(param_start_matrix))%dopar%{
  rw.sd_call <- rep(list(0.02),times=U-1)
  names(rw.sd_call) <- lapply((1:U)[-u],function(x)paste0("rho",x))
  ibpf(b,
    params=param_start_matrix[s,],
    Nbpf=switch(i,2,50),Np=switch(i,20,1000),
    rw.sd=do.call(rw.sd,rw.sd_call),
    unitParNames="rho",
    sharedParNames=NULL,
    block_size=2,
    spat_regression=0,
...
The profile took \Sexpr{myround(as.double(prof1time,units="mins"),2)} mins using \Sexpr{cores} computing cores.
Likelihood evaluation took \Sexpr{myround(as.double(prof1eval_time,units="mins"),2)}, shared between BPF and PF.

<<ibpf-prof-rho-plot, fig.height=8, fig.width=6, out.width="5.5in", fig.cap = "Top: profile for $\\rho_1$ using an IBPF search with likelihood computed using BPF, for $K=2$ blocks each having 2 units. Middle: Exact profile (dashed red line) and the same IBPF search with likelihood computed exactly using the Kalman filter. Bottom: The same IBPF search with likelihood computed using the particle filter.",eval=T,echo=F>>= 
...
ibpf_prof_sig_searches <- foreach(s=1:nrow(param_sig_start_matrix))%dopar%{
  rw.sd_call <- rep(list(0.02),times=U-1)
  names(rw.sd_call) <- lapply((1:U)[-u],function(x)paste0("sigma",x))
  ibpf(b_sig,
...
The $\sigma_1$ profile took \Sexpr{myround(as.double(prof_sig1time,units="mins"),2)} mins using \Sexpr{cores} computing cores.
Likelihood evaluation took \Sexpr{myround(as.double(prof_sig1eval_time,units="mins"),2)}, shared between BPF and PF.

<<ibpf-prof-sig-plot, fig.height=8, fig.width=6, out.width="5.5in", fig.cap = "Top: profile for $\\sigma_1$ using an IBPF search with likelihood computed using BPF, for $K=2$ blocks each having 2 units. Middle: Exact profile (dashed red line) and the same IBPF search with likelihood computed exactly using the Kalman filter. Bottom: The same IBPF search with likelihood computed using the particle filter.",eval=T,echo=F>>= 
...
These results show that there is noticeable bias in using BPF for likelihood evaluation for this model.
This bias leads to parameter estimation bias for IBPF.
The same methods can be used to assess BPF and IBPF for a metapopulation model.
...
he10_model <- he10(U=4,dt=1/365,Tmax=switch(i,1964,1964),
...
m_seed <- 27
# plot(simulate(he10_model,seed=1))
m <- simulate(he10_model,seed=m_seed)
@

<<model-he10-eval,eval=T,echo=F,fig.height=4, fig.width=6, out.width="6.5in", fig.cap = "Weekly measles case report data for four UK towns.">>=
m_dir <- paste0("m_",i,"/")
...
par(mfrow=c(2,1))
plot(he10_model,log=T)
@

<<he10-sim-plot,echo=F,fig.height=4, fig.width=6, out.width="6.5in", fig.cap = "Simulated data from the \\code{he10} model for four towns">>=
plot(m,log=T)
@
...
m_prof_R0_searches <- foreach(s=1:nrow(param_R0_start_matrix))%dopar%{
  rw.sd_call <- rep(list(0.02),times=U-1)
  names(rw.sd_call) <- lapply((1:U)[-m_R0_prof_u],function(x)paste0("R0",x))
  ibpf(m_R0,
...
The $R0_\Sexpr{m_R0_prof_u}$ profile took \Sexpr{myround(as.double(m_prof_R0_time,units="mins"),2)} mins using \Sexpr{cores} computing cores.
Likelihood evaluation took \Sexpr{myround(as.double(m_prof_R0_eval_time,units="mins"),2)}, shared between BPF and PF.

<<m-prof-R0-plot, fig.height=8, fig.width=6, out.width="5.5in", fig.cap = "Top: profile for $R0_1$ using an IBPF search with likelihood computed using BPF, for $K=4$ blocks each having 1 unit. Bottom: The same IBPF search with likelihood computed using the particle filter.",eval=T,echo=F>>= 
...
m_g <- simulate(he10_model,seed=m_seed)
if(any(obs(m_g)-obs(m_R0) != 0)) stop("m_g and m_R0 should have the same synthetic data")
})
@
...
<<m_slice_g_plot,eval=T,echo=F,fig.height=4, fig.width=6, out.width="5.5in", fig.cap = "Slice for $g_4$ through the true parameter vector, using BPF evaluation with $K=4$ blocks each having 1 unit.">>=
...
Figure~\ref{fig:m_slice_g_plot} suggests that the BPF likelihood approximation leads to a low-bias estimate of $g_u$ when compared with Figures~\ref{fig:ibpf-prof-rho-plot} and~\ref{fig:ibpf-prof-sig-plot}.
CHECK THAT THIS MAKES SENSE IN THE CONTEXT OF THE FINAL RESULTS.

The $g_\Sexpr{m_g_prof_u}$ slice took \Sexpr{myround(as.double(m_slice_g_time,units="mins"),2)} mins using \Sexpr{cores} computing cores.

<<m-prof-g,eval=F,echo=F>>=
stew(file=paste0(m_dir,"m_prof_g.rda"),seed=722,{
m_prof_g_vals <- seq(from=200,to=1800,length=switch(i,5,20))
tic <- Sys.time()
reps <- switch(i,2,10)
param_g_start_matrix <- matrix(coef(m_g),byrow=TRUE,
  ncol=length(coef(m_g)),nrow=reps*length(m_prof_g_vals),
  dimnames=list(NULL,names(coef(m_g))))
param_g_start_matrix[,paste0("g",m_g_prof_u)] <- rep(m_prof_g_vals,each=reps)
m_prof_g_searches <- foreach(s=1:nrow(param_g_start_matrix))%dopar%{
  rw.sd_call <- rep(list(0.02),times=U-1)
  names(rw.sd_call) <- lapply((1:U)[-m_g_prof_u],function(x)paste0("g",x))
  ibpf(m_g,
    params=param_g_start_matrix[s,],
    Nbpf=switch(i,2,50),Np=switch(i,10,2000),
    rw.sd=do.call(rw.sd,rw.sd_call),
    unitParNames="g",
    sharedParNames=NULL,
...
m_prof_g_time <- toc-tic
@


<<m-prof-g-lik,eval=F,echo=F>>=
stew(file=paste0(m_dir,"m_prof_g",m_g_prof_u,"_lik.rda"),seed=868,{
tic <- Sys.time()
m_bpf_g_eval <- foreach(mfit=m_prof_g_searches,.combine=cbind) %dopar% {
  replicate(switch(i,2,10),
    logLik(bpfilter(mfit,block_size=1,Np=switch(i,10,5000)))
  )
}
m_pf_g_eval <- foreach(mfit=m_prof_g_searches,.combine=cbind) %dopar% {
  replicate(switch(i,2,10),
    logLik(pfilter(mfit,Np=switch(i,10,5000)))
  )
}
...
m_prof_g_eval_time <- toc-tic
@

%% The $g_\Sexpr{m_g_prof_u}$ profile took \Sexpr{myround(as.double(m_prof_g_time,units="mins"),2)} mins using \Sexpr{cores} computing cores.
%% Likelihood evaluation took \Sexpr{myround(as.double(m_prof_g_eval_time,units="mins"),2)}, shared between BPF and PF.

<<m-prof-g-plot,eval=F,echo=F, fig.height=8, fig.width=6, out.width="5.5in", fig.cap = "Top: profile for $g_4$ using an IBPF search with likelihood computed using BPF, for $K=4$ blocks each having 1 unit. Bottom: The same IBPF search with likelihood computed using the particle filter.">>= 

par(mfrow=c(2,1))
par(mai=c(0.9,0.9,0.1,0.2))

##### bpf evaluation
m_p2_g_bpf <- apply(matrix(apply(m_bpf_g_eval,2,mean),nrow=reps),2,max)
bpf_prof_g_range <- c(max(m_p2_g_bpf)-10,max(m_p2_g_bpf))
plot(y=m_p2_g_bpf,x=m_prof_g_vals,ylim=bpf_prof_g_range,
  ylab="Log-likelihood",xlab="")
m_p2_g_bpf_mcap <- mcap(logLik=m_p2_g_bpf,parameter=m_prof_g_vals)
lines(m_p2_g_bpf_mcap$fit$parameter,m_p2_g_bpf_mcap$fit$smoothed)

abline(v=m_p2_g_bpf_mcap$ci)

######## pf evaluation
m_p2_g_pf <- apply(matrix(apply(m_pf_g_eval,2,mean),nrow=reps),2,max)
pf_prof_g_range <- c(max(m_p2_g_pf)-10,max(m_p2_g_pf))
plot(y=m_p2_g_pf,x=m_prof_g_vals,ylim=pf_prof_g_range,
  ylab="Log-likelihood",xlab="g")
m_p2_g_pf_mcap <- mcap(logLik=m_p2_g_pf,parameter=m_prof_g_vals)
lines(m_p2_g_pf_mcap$fit$parameter,m_p2_g_pf_mcap$fit$smoothed)

abline(v=m_p2_g_pf_mcap$ci)
