...
library(spatPomp)
i <- 2
b <- bm2(U=4,N=switch(i,20,200),unit_specific_names="rho")
plot(b)
@
...
stew(file=paste0(out_dir,"logLik.rda"),seed=10,{
<<bm_logLik_code>>
...
stew(file=paste0(out_dir,"kf_mle.rda"),seed=999,{
...
stew(file=paste0(out_dir,"kf_prof.rda"),seed=999,{
...
stew(file=paste0(out_dir,"ibpf_mle.rda"),seed=999,{
<<ibpf-mle-code>>
})

<<ibpf-mle-lik,eval=T,echo=F>>=
stew(file=paste0(out_dir,"ibpf_mle_lik.rda"),seed=888,{
...
ibpf_bpf_eval <- foreach(bm2fit=ibpf_mle_searches,.combine=cbind) %dopar% {
  replicate(switch(i,5,20),
    logLik(bpfilter(bm2fit,block_size=2,Np=switch(i,20,1000)))
  )
}
})
@
...
stew(file=paste0(out_dir,"prof1.rda"),seed=722,{
tic <- Sys.time()
u <- 1
rho_start <- seq(from=0.2,to=0.8,length=U)
reps <- switch(i,3,10)
...
stew(file=paste0(out_dir,"prof1_lik.rda"),seed=868,{
tic <- Sys.time()
...
ibpf_pf_eval <- foreach(bm2fit=ibpf_prof_searches,.combine=cbind) %dopar% {
  replicate(switch(i,5,20),
    logLik(pfilter(bm2fit,Np=switch(i,20,1000)))
  )
}
toc <- Sys.time()
})
...
set.seed(20)
b_sig <- bm2(U=4,N=switch(i,20,200),unit_specific_names="sigma")
# b_sig has the same data as b
# plot(b_sig)
...
stew(file=paste0(out_dir,"kf_mle_sig.rda"),seed=256,{
...
stew(file=paste0(out_dir,"kf_prof_sig.rda"),seed=512,{
...
stew(file=paste0(out_dir,"prof_sig1.rda"),seed=722,{
tic <- Sys.time()
u <- 1
...
stew(file=paste0(out_dir,"prof_sig1_lik.rda"),seed=868,{
tic <- Sys.time()
...
he10_model <- he10(U=4,dt=1/365,Tmax=1964,expandedParNames=c("R0"),
  basic_params = c(
...
m <- simulate(he10_model,seed=9)
@

<<model-he10-eval,eval=T,echo=F>>=
stew(file=paste0(out_dir,"m-sim.rda"),{
<<model-he10-code>>
})
...
stew(file=paste0(out_dir,"m-logLik.rda"),seed=10,{
  tic
...
<<m-prof-sig,eval=T,echo=F>>=
stew(file=paste0(out_dir,"m_prof_R01.rda"),seed=722,{
m_prof_R0_init <- rep(30,U)
names(m_prof_R0_init) <- paste0("R0",1:U)
m_prof_R0_vals <- seq(from=15,to=45,length=switch(i,5,20))
tic <- Sys.time()
m_R0 <- m
u <- 2
R0_start <- seq(from=15,to=45,length=U)
params_R0_start <- coef(m_R0)
params_R0_start[paste0("R0",1:U)] <- R0_start
reps <- switch(i,3,10)
param_R0_start_matrix <- matrix(params_R0_start,byrow=TRUE,
  ncol=length(coef(m_R0)),nrow=reps*length(m_prof_R0_vals),
  dimnames=list(NULL,names(coef(m_R0))))
param_R0_start_matrix[,paste0("R0",u)] <- rep(m_prof_R0_vals,each=reps)
m_prof_R0_searches <- foreach(s=1:nrow(param_R0_start_matrix))%dopar%{
  rw.sd_call <- rep(list(0.02),times=U-1)
  names(rw.sd_call) <- lapply((1:U)[-u],function(x)paste0("R0",x))
  ibpf(m_R0,
    params=param_R0_start_matrix[s,],
    Nbpf=switch(i,2,50),Np=switch(i,20,2000),
    rw.sd=do.call(rw.sd,rw.sd_call),
    unitParNames="R0",
    sharedParNames=NULL,
    block_size=1,
...
m_prof_R02time <- toc-tic
@


<<m-prof-sig-lik,eval=T,echo=F>>=
stew(file=paste0(out_dir,"m_prof_R0",u,"_lik.rda"),seed=868,{
tic <- Sys.time()
m_bpf_R0_eval <- foreach(mfit=m_prof_R0_searches,.combine=cbind) %dopar% {
  replicate(switch(i,5,10),
    logLik(bpfilter(mfit,block_size=1,Np=switch(i,20,5000)))
  )
}
m_pf_R0_eval <- foreach(mfit=m_prof_R0_searches,.combine=cbind) %dopar% {
  replicate(switch(i,5,10),
    logLik(pfilter(mfit,Np=switch(i,20,5000)))
  )
}
toc <- Sys.time()
})
m_prof_R02eval_time <- toc-tic
@

The $\R0_1$ profile took \Sexpr{myround(as.double(m_prof_R02time,units="mins"),2)} mins using \Sexpr{cores} computing cores.
Likelihood evaluation took \Sexpr{myround(as.double(m_prof_R02eval_time,units="mins"),2)}, shared between BPF and PF.

<<m-prof-sig1-plot, fig.height=8, fig.width=6, out.width="5.5in", fig.cap = "Top: profile for $R0_1$ using an IBPF search with likelihood computed using BPF, for $K=4$ blocks each having 1 unit. Middle: Exact profile (dashed red line) and the same IBPF search with likelihood computed exactly using the Kalman filter. Bottom: The same IBPF search with likelihood computed using the particle filter.",eval=T,echo=F>>= 

par(mfrow=c(2,1))
par(mai=c(0.9,0.9,0.1,0.2))

##### window to avoid problematic particle filter behavior at extremes
prof_include <- (m_prof_R0_vals > 20) & (m_prof_R0_vals < 40)
